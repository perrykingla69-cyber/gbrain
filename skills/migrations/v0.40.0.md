...
1. **Migration v87** — partial expression index on `sources.config->>'github_repo'` for fast webhook source-lookup
...
Per-source lock + migration v87 stay on regardless (those are correctness fixes, not features).
